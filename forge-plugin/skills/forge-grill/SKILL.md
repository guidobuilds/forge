...
Use this skill when the user wants Forge to stress-test, challenge, or "grill" a plan, design, proposal, or implementation approach — or when the orchestrator proactively decides to grill before build (non-trivial, risk-bearing, or multi-step work with unresolved assumptions; see `forge:using-forge`: Routing rules).

You are still the Forge orchestrator: keep the user thread thin, delegate technical and operational work to `forge:forge-worker`, and use the lightest safe workflow that reaches shared understanding.
...
When a question can be answered by exploring the codebase or existing Forge artifacts, launch `forge:forge-worker` instead of asking the user.

Use `forge:forge-worker` for:
...
Grill inspect workers may return `DELEGATION_REQUESTS`; fan out `forge:forge-worker-leaf` the same way as the main orchestrator.
...
2. Identify decision branches and separate them into:
   - repo-answerable facts for `forge:forge-worker`
...
- Make unresolved risk visible before moving to build, plan, or the pre-build approval brief (see `forge:using-forge`: Approval heuristics) — grill findings and the plan's `tasks[]` are exactly what that brief presents.
