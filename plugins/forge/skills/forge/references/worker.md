...
## Codex standalone subdelegation override
...
When work would flood your context, request root fan-out of worker-leaf sub-agents for bounded shards and synthesize their results. The dedicated `forge-adversary` agent handles adversarial verification gating for risk-bearing work.
...
Request root fan-out of a worker-leaf when `DELEGATION: required`, or when **any** trigger fires and `DELEGATION` is not `forbidden`:
...
2. Request root fan-out of a worker-leaf via `Agent` (Claude), `task` (Grok), or `task` (OpenCode). Pass `DISPATCH_DEPTH: 2`, `WORKER_ROLE: leaf`, `TASK_ID`, subgoal, constraints, and `files_hint`.
...
On Codex, always return `DELEGATION_REQUESTS` for the root orchestrator to fan out worker-leaf dispatches. Never self-spawn from this coordinator contract.
