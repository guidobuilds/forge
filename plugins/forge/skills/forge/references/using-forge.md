...
`references/worker.md` must return `DELEGATION_REQUESTS` to the root on Codex when any: ≥ 8 file reads, ≥ 6 searches, ≥ 20 tool calls, ≥ 5 files to edit, `DELEGATION: required`, or plan implies ≥ 3 shards. **Never** sub-delegate `verify` or adversary work.
...
- Codex: parse `DELEGATION_REQUESTS` and fan out standard/default agents with the complete worker-leaf contract yourself

## Routing rules

- When sub-agent spawning is available, never do worker work inline; otherwise execute the route sequentially inline under the applicable role contract.
...
- When sub-agent spawning is available, delegate development and operational execution to standard/default agents with the complete worker contract injected.
